...
- **高水位 (High Watermark, 80%)**：当 `writeChan` 积压超过了 **10240** 个插槽（约 **320MB**）的 80% (8192) 时，进入“读拦截”模式。通过 TCP 的窗口自适应机制（Window Zero），自然地让发送端（客户端）减速，**防止内存膨胀导致的 OOM**。
- **低水位 (Low Watermark, 20%)**：当后端消费了大部分积压数据，水位降至 20% (2048) 以下时，从管道消费端调用 `gnet.Conn.Wake()` 唤醒事件循环并恢复读取。
- **下行背压控制 (Downlink Control)**：在 `RelayBack` 协程中持续监控 `OutboundBuffered`（待发缓冲区）。如果客户端由于带宽原因接收缓慢导致数据堆积超过 **4MB**，则自动挂起后端读取。这能反向触发远程服务器的 TCP 拥塞控制，实现了全链路、闭环的流量平衡，从根本上防止了“发得太快，接得太慢”导致的代理进程 OOM。
